...
Employee.prototype.giveSalary  = function() {
  return document.write("<p>" +this.name + " " + this.lastName + " отримує " + this.getSalary() + " шекєлей "+ "</p>" )
}
...
Department.prototype.giveSalary = function() {
 ...
 function main() {
	const dev = new Developer("Kim", "Kitsuragi", 1000, 5);
  	const dev2 = new Developer("Goracy", "Kubek", 1000, 1);
 	 const dev3 = new Developer("Rene", "Aronoux", 1200, 9);
 	 const dev4 = new Developer("Harrier", "Du Bois", 1100, 9);
  const des = new Designer("Raul", "Kortenaer", 1200, 6, 0.5);
  const des1 = new Designer("Ellis", "Kortenaer", 1000, 6, 0.7);
  const manager = new Manager("Evrart", "Claire", 1400, 5, [dev, dev2, dev3, dev4,des,des1]);
